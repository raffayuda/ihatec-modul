...
export function PdfThumbnail({ url, fallback }: PdfThumbnailProps) {
    ...
    return (
        <div className="dark:border-neutral-850 relative flex h-32 w-24 items-center justify-center overflow-hidden rounded-lg border border-neutral-200 bg-neutral-50 shadow-sm transition-shadow hover:shadow-md dark:bg-neutral-900">
            {loading && (
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/80 font-medium dark:bg-neutral-950/80">
...
function FileDropZone({ onFileSelect, selectedFile, existingFileName, existingFileSize, existingFileUrl }: FileDropZoneProps) {
        ...
        return (
            <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50/50 p-3.5 dark:border-emerald-900/40 dark:bg-emerald-950/20">
                ...
                <button
                    ...
                    className="flex size-6 items-center justify-center rounded text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-700"
                >
                    ...
                    <div className="flex items-center gap-1">
                        {existingFileUrl && (
                            <a
                                ...
                                className="flex size-7 items-center justify-center rounded text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/40"
                                title="Preview"
                            >
    ...
    return (
        <div>
            ...
            >
                <div
                    className={`flex size-10 items-center justify-center rounded-xl ${isDragOver ? 'bg-blue-100 text-blue-600' : 'bg-neutral-100 text-neutral-400 dark:bg-neutral-800'}`}
                >
                ...
                <div>
                    ...
                    <p className="mt-0.5 text-[10px] text-neutral-400">Hanya format PDF, maks. 20 MB</p>
                </div>
            </div>
            <input ref={inputRef} type="file" accept="application/pdf" className="hidden" onChange={handleFileChange} />
        </div>
...
export default function Pengajuan() {
    const {
        ...
        flash,
        trainingTypes,
        jenisKebutuhanOptions,
        bahasaPengantarOptions,
        isDriveConnected = true,
    } = usePage<PengajuanProps>().props;
    ...
    const jenisKebutuhanList = jenisKebutuhanOptions || ['Pelatihan Inhouse', 'Pelatihan Internal', 'Seminar'];

    const bahasaPengantarList = bahasaPengantarOptions || ['Indonesia', 'English', 'Arab', 'Mandarin'];
            ...
            <div className="flex h-full flex-1 flex-col gap-6 bg-neutral-50/60 p-6 dark:bg-neutral-900/10">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">
                        {role === 'User'
                            ? 'Ajukan dan pantau permintaan modul khusus Anda.'
                            : 'Kelola seluruh permintaan modul khusus dari pengguna.'}
                    </p>
                </div>
                ...
                {localToast && (
                    <div
                        className={`animate-in fade-in slide-in-from-bottom-5 fixed right-5 bottom-5 z-[100] flex items-center gap-2 rounded-xl border p-4 text-sm font-semibold shadow-lg duration-300 ${
                    ...
                    >
                        {localToast.type === 'success' ? (
                            ...
                            <AlertTriangle className="dark:text-rose-450 size-4.5 text-rose-600" />
                        )}
                ...
                {!isDriveConnected && (
                    <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50/50 p-4 text-amber-800 shadow-sm dark:border-amber-900/40 dark:bg-amber-950/10 dark:text-amber-400">
                        <AlertTriangle className="mt-0.5 size-5 flex-shrink-0 text-amber-600 dark:text-amber-500" />
                        <div className="font-sans text-xs leading-normal font-semibold">
                            <strong>Pemberitahuan:</strong> Akun Google Drive belum terhubung. Anda masih dapat mengajukan draf/permintaan modul,
                            tetapi pengajuan tersebut tidak akan bisa disetujui (Approve) oleh Manager PD sampai akun Google Drive terhubung. Harap
                            hubungi Administrator untuk menautkan akun Google Drive.
                        </div>
                    </div>
                )}
                ...
                {role === 'User' &&
                    submissions.some(
                        (item) => item.status === 'Selesai' && item.type === 'Kebutuhan Khusus' && !dismissedNotifications.includes(item.id),
                    ) && (
                        <div className="space-y-2.5">
                            {submissions
                                .filter(
                                    (item) =>
                                        item.status === 'Selesai' && item.type === 'Kebutuhan Khusus' && !dismissedNotifications.includes(item.id),
                                )
                                .map((item) => (
                                    <div
                                        key={item.id}
                                        className="flex items-start justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50/80 p-4 text-xs font-medium text-emerald-800 shadow-sm dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-400"
                                    >
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="mt-0.5 font-sans font-normal text-emerald-600/95 dark:text-emerald-400/90">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {item.link_modul && (
                                                <a
                                                    ...
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="inline-flex items-center gap-1 rounded-lg border border-emerald-200 bg-white/80 px-2.5 py-1.5 font-sans font-bold text-emerald-700 hover:underline dark:border-emerald-900/50 dark:bg-emerald-950/50 dark:text-emerald-300"
                                                >
                                            ...
                                            <button
                                                ...
                                                className="flex size-7 cursor-pointer items-center justify-center rounded-lg border border-emerald-200 font-sans text-emerald-600 hover:bg-emerald-100 dark:border-emerald-900/50 dark:hover:bg-emerald-950"
                                                title="Tutup"
                                            >
                ...
                <div className="space-y-6">
                        ...
                        {[
                            {
                                ...
                                value: stats.total,
                                icon: FileText,
                                color: 'text-blue-600 bg-blue-50 dark:bg-blue-950/50 dark:text-blue-400 border border-blue-200 dark:border-blue-900',
                            },
                            {
                                label: 'Process',
                                value: stats.process,
                                icon: RefreshCw,
                                color: 'text-blue-500 bg-blue-50 dark:bg-blue-950/50 dark:text-blue-400 border border-blue-200 dark:border-blue-900',
                            },
                            {
                                label: 'Menunggu',
                                value: stats.waiting,
                                icon: Clock,
                                color: 'text-purple-650 bg-purple-50 dark:bg-purple-950/50 dark:text-purple-400 border border-purple-200 dark:border-purple-900',
                            },
                            {
                                label: 'Done',
                                value: stats.done,
                                icon: CheckCircle2,
                                color: 'text-emerald-650 bg-emerald-50 dark:bg-emerald-950/50 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900',
                            },
                            {
                                label: 'Hold',
                                value: stats.hold,
                                icon: AlertTriangle,
                                color: 'text-amber-600 bg-amber-50 dark:bg-amber-950/50 dark:text-amber-400 border border-amber-200 dark:border-amber-900',
                            },
                            {
                                label: 'Cancel',
                                value: stats.cancel,
                                icon: XCircle,
                                color: 'text-rose-600 bg-rose-50 dark:bg-rose-950/50 dark:text-rose-400 border border-rose-200 dark:border-rose-900',
                            },
                        ].map((m) => (
                            <Card key={m.label} className="border-neutral-200/80 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
                                <CardContent className="flex items-start gap-2 p-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-[10px] font-semibold tracking-wider text-neutral-400 uppercase">{m.label}</div>
                                    </div>
                                </CardContent>
                            </Card>
                        ))}
                    ...
                    <Card className="overflow-hidden border-neutral-200/80 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
                        {/* Filters */}
                        <div className="flex flex-col gap-3 border-b border-neutral-100 p-4 md:flex-row md:items-center md:justify-between dark:border-neutral-800">
                            <div className="relative max-w-sm flex-1">
                                ...
                                <input
                                    ...
                                    className="h-9 w-full rounded-lg border border-neutral-200 bg-white pr-4 pl-9 text-xs text-neutral-900 outline-none placeholder:text-neutral-400 focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                />
                            </div>
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                <div className="w-44">
                                    <SearchableSelect
                                        ...
                                        options={['Semua Tipe', 'Kebutuhan Khusus']}
                                    />
                                </div>
                                <Button
                                    ...
                                    variant="outline"
                                    size="sm"
                                    className="h-9 rounded-lg border-neutral-200 px-3 text-xs font-semibold dark:border-neutral-800"
                                >
                                ...
                                <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                                    {currentItems.length === 0 ? (
                                        ...
                                        currentItems.map((item) => (
                                            ...
                                            >
                                                <td className="px-5 py-4 font-mono text-[10px] font-semibold whitespace-nowrap text-neutral-500 dark:text-neutral-400">
                                                    {item.id}
                                                </td>
                                                <td className="max-w-[160px] px-5 py-4" onClick={(e) => e.stopPropagation()}>
                                                    <button
                                                        onClick={() => setDetailItem(item)}
                                                        className="line-clamp-2 cursor-pointer text-left leading-tight font-semibold text-neutral-900 hover:text-blue-600 dark:text-neutral-100 dark:hover:text-blue-400"
                                                    >
                                                        {item.title}
                                                    </button>
                                                </td>
                                                ...
                                                <td className="px-5 py-4 font-medium whitespace-nowrap text-neutral-600 dark:text-neutral-400">
                                                    {item.applicant}
                                                </td>
                                                <td className="px-5 py-4 font-medium whitespace-nowrap text-neutral-500 dark:text-neutral-400">
                                                    {item.deadline}
                                                </td>
                                                {/* File column */}
                                                    ...
                                                    {item.fileName ? (
                                                        <div className="flex items-center gap-1.5">
                                                            ...
                                                            <div className="min-w-0">
                                                                <p className="max-w-[90px] truncate text-[10px] font-semibold text-neutral-700 dark:text-neutral-300">
                                                                    {item.fileName}
                                                                </p>
                                                        ...
                                                        <div className="flex items-center gap-1.5">
                                                            ...
                                                            <div className="min-w-0">
                                                                <a
                                                                    ...
                                                                    target="_blank"
                                                                    rel="noopener noreferrer"
                                                                    className="block max-w-[120px] truncate text-[10px] font-semibold text-blue-600 hover:underline dark:text-blue-400"
                                                                >
                                                                    Buka Link Modul
                                                                </a>
                                                            </div>
                                                        ...
                                                        <span className="font-sans text-[10px] font-medium text-neutral-400 italic">
                                                            Tidak ada dokumen
                                                        </span>
                                                    ) : (
                                                        <button
                                                            ...
                                                            className="flex cursor-pointer items-center gap-1 text-[10px] font-semibold text-neutral-400 hover:text-blue-600 dark:hover:text-blue-400"
                                                        >
                                                ...
                                                <td className="px-5 py-4">
                                                    <Badge
                                                        variant="outline"
                                                        className={`rounded-md border-0 px-2 py-0.5 text-[10px] font-semibold ${STATUS_COLORS[item.status] ?? ''}`}
                                                    >
                                                ...
                                                <td className="px-5 py-4 text-center" onClick={(e) => e.stopPropagation()}>
                                                    <DropdownMenu>
                                                        <DropdownMenuTrigger asChild>
                                                            <button className="mx-auto flex size-7 cursor-pointer items-center justify-center rounded text-neutral-500 hover:bg-neutral-100 dark:text-neutral-400 dark:hover:bg-neutral-800">
                                                        ...
                                                        <DropdownMenuContent align="end" className="w-48 text-xs">
                                                            ...
                                                            {role === 'User' && canEdit(item.status) && (
                                                                <DropdownMenuItem
                                                                    onClick={() => handleCancelByUser(item)}
                        ...
                        {/* Pagination */}
                        <div className="flex flex-col gap-3 border-t border-neutral-100 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-neutral-800">
                            <span className="text-xs font-medium text-neutral-500 dark:text-neutral-400">
                                Menampilkan {indexOfFirst + 1}–{Math.min(indexOfLast, filteredSubmissions.length)} dari {filteredSubmissions.length}{' '}
                                pengajuan
                            </span>
                            <div className="flex items-center gap-1.5">
                                <button
                                ...
                                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                    <button
                                        ...
                                        className={`flex size-7 items-center justify-center rounded border text-xs font-semibold ${page === currentPage ? 'border-blue-600 bg-blue-600 text-white' : 'border-neutral-200 bg-white text-neutral-600 hover:bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-400'}`}
                                    >
                                        {page}
                                    </button>
                                ))}
            ...
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                <DialogContent className="max-h-[85vh] max-w-xl overflow-y-auto">
                    ...
                    <form onSubmit={handleCreate} className="mt-2 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 rounded-xl border border-neutral-200/60 bg-neutral-50 p-3.5 dark:border-neutral-800 dark:bg-neutral-900/40">
                            <div className="col-span-2 mb-1 text-xs font-bold tracking-wider text-neutral-400 uppercase">Informasi Umum</div>
                            <div>
                                <label className="mb-1 block font-sans text-[10px] font-bold tracking-wider text-neutral-500 uppercase dark:text-neutral-400">
                                    Nama Pengaju
                                </label>
                                <input
                                    type="text"
                                    ...
                                    disabled
                                    className="w-full rounded-lg border border-neutral-200 bg-neutral-100/80 px-3 py-1.5 text-xs text-neutral-500 outline-none dark:bg-neutral-800/80 dark:text-neutral-400"
                                />
                            </div>
                            <div>
                                <label className="mb-1 block font-sans text-[10px] font-bold tracking-wider text-neutral-500 uppercase dark:text-neutral-400">
                            ...
                            <div>
                                ...
                                {createData.type === 'Kebutuhan Khusus' && !isDateValid(createData.deadline) && createData.deadline && (
                                    <p className="mt-1 flex items-center gap-0.5 font-sans text-[9px] leading-none text-rose-500">
                                        <AlertTriangle className="size-2.5 flex-shrink-0" />
                                        Harus minimal 14 hari ke depan.
                                    </p>
                                )}
                                {createErrors.deadline && <p className="mt-1 font-sans text-[10px] text-rose-500">{createErrors.deadline}</p>}
                            </div>
                        </div>

                        {/* ── CONDITIONAL SECTION: MODUL BARU ── */}
                        {createData.type === 'Modul Baru' && (
                            <div className="space-y-3 rounded-xl border border-blue-100 bg-blue-50/20 p-3.5 dark:border-blue-900/40 dark:bg-blue-950/5">
                                ...
                                <div>
                                    ...
                                    <SearchableSelect
                                        ...
                                        onChange={(val) => setCreateData({ ...createData, program: val })}
                                        options={trainingTypeList}
                                        nullLabel="-- Pilih Jenis Pelatihan --"
                                        required={createData.type === 'Modul Baru'}
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                        placeholder="e.g. Interpretasi Sistem ISO"
                                        required
                                    />
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            min="1"
                                            value={createData.training_days}
                                            onChange={(e) => setCreateData({ ...createData, training_days: e.target.value })}
                                            className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                            placeholder="e.g. 3"
                                        />
                                    </div>
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                        placeholder="Jelaskan kebutuhan khusus atau background permintaan..."
                                    />
                                </div>
                            </div>
                        )}
                        ...
                        {createData.type === 'Revisi Modul' && (
                            <div className="space-y-3 rounded-xl border border-violet-100 bg-violet-50/20 p-3.5 dark:border-violet-900/40 dark:bg-violet-950/5">
                                <div className="mb-1 text-xs font-bold tracking-wider text-violet-600 uppercase dark:text-violet-400">
                                    Detail Modul Existing / Revisi
                                </div>
                                ...
                                <div>
                                    ...
                                    <SearchableSelect
                                        value={createData.related_module_id}
                                        onChange={(val) => handleRelatedModuleChange(val)}
                                        options={availableModules.map((m) => ({
                                            value: String(m.id),
                                            label: `${m.code} — ${m.title} (Rev. ${m.revision})`,
                                        }))}
                                        nullLabel="-- Pilih Modul --"
                                        required={createData.type === 'Revisi Modul'}
                                    />
                                </div>

                                {createData.related_module_id && (
                                    <div className="grid grid-cols-3 gap-2 rounded-lg border border-neutral-200/50 bg-neutral-100/60 p-2.5 text-[11px] text-neutral-600 dark:border-neutral-700/50 dark:bg-neutral-800/60 dark:text-neutral-400">
                                        <div>
                                            <span className="block text-[9px] font-bold tracking-wider text-neutral-400 uppercase">Kode Modul</span>
                                            <span>{availableModules.find((m) => m.id === parseInt(createData.related_module_id, 10))?.code}</span>
                                        </div>
                                        <div className="col-span-2">
                                            <span className="block font-sans text-[9px] font-bold tracking-wider text-neutral-400 uppercase">
                                                Judul & Versi Aktif
                                            </span>
                                            <span className="line-clamp-1">
                                                {availableModules.find((m) => m.id === parseInt(createData.related_module_id, 10))?.title} (Rev.{' '}
                                                {availableModules.find((m) => m.id === parseInt(createData.related_module_id, 10))?.revision})
                                            </span>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div>
                                    ...
                                    <textarea
                                        value={createData.revision_reason}
                                        onChange={(e) => setCreateData({ ...createData, revision_reason: e.target.value })}
                                        rows={2}
                                        className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                        placeholder="Alasan mengapa modul ini perlu direvisi..."
                                        required={createData.type === 'Revisi Modul'}
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                        placeholder="Rincian materi/bab yang diubah..."
                                        required={createData.type === 'Revisi Modul'}
                                    />
                                </div>
                            </div>
                        ...
                        {createData.type === 'Kebutuhan Khusus' && (
                            <div className="space-y-3 rounded-xl border border-teal-100 bg-teal-50/20 p-3.5 dark:border-teal-900/40 dark:bg-teal-950/5">
                                <div className="mb-1 text-xs font-bold tracking-wider text-teal-600 uppercase dark:text-teal-400">
                                    Detail Kebutuhan Khusus
                                </div>
                                        ...
                                        <SearchableSelect
                                            ...
                                            onChange={(val) =>
                                                setCreateData({
                                                    ...createData,
                                                    jenis_kebutuhan: val,
                                                    nama_instansi: val !== 'Pelatihan Inhouse' ? '' : createData.nama_instansi,
                                                })
                                            }
                                ...
                                <div>
                                    <label className="mb-1 block font-sans text-[10px] font-bold tracking-wider text-neutral-500 uppercase dark:text-neutral-400">
                                        Keterangan / Detail Kebutuhan
                                    </label>
                                    <textarea
                                        value={createData.description}
                                        onChange={(e) =>
                                            setCreateData({ ...createData, description: e.target.value, keterangan_kebutuhan: e.target.value })
                                        }
                                        rows={4}
                                        className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                        ...
                        {(createData.type === 'Modul Baru' || createData.type === 'Revisi Modul') && (
                            <div>
                                <label className="mb-1.5 block font-sans text-xs font-semibold text-neutral-600 dark:text-neutral-400">
                                    Dokumen PDF{' '}
                                    {createData.type === 'Revisi Modul' ? (
                                ...
                                {createErrors.file && <p className="mt-1 font-sans text-xs text-rose-500">{createErrors.file}</p>}
                            </div>
                        )}
                    ...
                    <form onSubmit={handleEdit} className="mt-2 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 rounded-xl border border-neutral-200/60 bg-neutral-50 p-3.5 dark:border-neutral-800 dark:bg-neutral-900/40">
                            ...
                            <div>
                                ...
                                <SearchableSelect
                                    ...
                                    disabled={role === 'User'}
                                    options={role !== 'User' ? ['Modul Baru', 'Revisi Modul', 'Kebutuhan Khusus'] : ['Kebutuhan Khusus']}
                                />
                            </div>
                            <div>
                                <label className="mb-1 block font-sans text-[10px] font-bold tracking-wider text-neutral-500 uppercase dark:text-neutral-400">
                                ...
                                {editForm.data.type === 'Kebutuhan Khusus' &&
                                    !['Admin', 'Staf PD'].includes(role) &&
                                    !isDateValid(editForm.data.deadline) &&
                                    editForm.data.deadline && (
                                        <p className="mt-1 flex items-center gap-0.5 font-sans text-[9px] leading-none text-rose-500">
                                            <AlertTriangle className="size-2.5 flex-shrink-0" />
                                            Harus minimal 14 hari ke depan.
                                        </p>
                                    )}
                                {editForm.errors.deadline && <p className="mt-1 font-sans text-[10px] text-rose-500">{editForm.errors.deadline}</p>}
                            </div>
                        </div>
                            ...
                            <div className="space-y-3 rounded-xl border border-blue-100 bg-blue-50/20 p-3.5 dark:border-blue-900/40 dark:bg-blue-950/5">
                                ...
                                <div>
                                    ...
                                    <SearchableSelect
                                        ...
                                        options={trainingTypeList}
                                        nullLabel="-- Pilih Jenis Pelatihan --"
                                        required={editForm.data.type === 'Modul Baru'}
                                    />
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            type="number"
                                            min="1"
                                            value={editForm.data.training_days}
                                            onChange={(e) => editForm.setData('training_days', e.target.value)}
                                            className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 font-sans text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                        />
                                    </div>
                                </div>
                        ...
                        {editForm.data.type === 'Revisi Modul' && (
                            <div className="space-y-3 rounded-xl border border-violet-100 bg-violet-50/20 p-3.5 dark:border-violet-900/40 dark:bg-violet-950/5">
                                ...
                                {editForm.data.related_module_id && (
                                    <div className="grid grid-cols-3 gap-2 rounded-lg border border-neutral-200/50 bg-neutral-100/60 p-2.5 text-[11px] text-neutral-600 dark:border-neutral-700/50 dark:bg-neutral-800/60 dark:text-neutral-400">
                                        <div>
                                            ...
                                            <span>
                                                {availableModules.find((m) => m.id === parseInt(editForm.data.related_module_id as string, 10))?.code}
                                            </span>
                                        </div>
                                        <div className="col-span-2">
                                            <span className="block font-sans text-[9px] font-bold tracking-wider text-neutral-400 uppercase">
                                            ...
                                            <span className="line-clamp-1">
                                                {
                                                    availableModules.find((m) => m.id === parseInt(editForm.data.related_module_id as string, 10))
                                                        ?.title
                                                }{' '}
                                                (Rev.{' '}
                                                {
                                ...
                                <div>
                                    ...
                                    <textarea
                                        value={editForm.data.revision_reason}
                                        onChange={(e) => editForm.setData('revision_reason', e.target.value)}
                                        rows={2}
                                        className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 font-sans text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                        required={editForm.data.type === 'Revisi Modul'}
                                    />
                                ...
                                <div className="space-y-3 rounded-xl border border-teal-100 bg-teal-50/20 p-3.5 dark:border-teal-900/40 dark:bg-teal-950/5">
                                            ...
                                            <SearchableSelect
                                                ...
                                                onChange={(val) =>
                                                    editForm.setData((data) => ({
                                                        ...data,
                                                        jenis_kebutuhan: val,
                                                        nama_instansi: val !== 'Pelatihan Inhouse' ? '' : data.nama_instansi,
                                                    }))
                                                }
                                                options={jenisKebutuhanList}
                                                nullLabel="-- Pilih Jenis --"
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) =>
                                                editForm.setData((data) => ({ ...data, judul_program: e.target.value, title: e.target.value }))
                                            }
                                            className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                                            placeholder="e.g. Inhouse Training ISO 9001:2015"
                                            required
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) =>
                                                editForm.setData((data) => ({
                                                    ...
                                                    keterangan_kebutuhan: e.target.value,
                                                }))
                                            }
                                            rows={4}
                                            className="w-full rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-xs outline-none focus:border-blue-500 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100"
                        ...
                        {(editForm.data.type === 'Modul Baru' || editForm.data.type === 'Revisi Modul') && (
                            <div>
                                ...
                                />
                                {(editForm.errors as Record<string, string>).file && (
                                    <p className="mt-1 font-sans text-xs text-rose-500">{(editForm.errors as Record<string, string>).file}</p>
                                )}
                            </div>
                        )}
            ...
            >
                <DialogContent className="max-w-md">
                    ...
                    <form onSubmit={handleProses} className="mt-2 space-y-4">
                        <div className="border-purple-250 space-y-3 rounded-xl border bg-purple-50/10 p-3.5 dark:border-purple-900/40 dark:bg-purple-950/5">
                            <div>
                                <label className="mb-1.5 block font-sans text-[10px] font-bold tracking-wider text-purple-600 uppercase dark:text-purple-400">
                                    Status *
                                </label>
                                <SearchableSelect
                                    ...
                                    required
                                />
                                {prosesForm.errors.status && <p className="mt-1 font-sans text-[10px] text-rose-500">{prosesForm.errors.status}</p>}
                            </div>

                            {/* Link Modul - Visible only if Selesai */}
                            {prosesForm.data.status === 'Selesai' && (
                                <div>
                                    <label className="mb-1.5 block font-sans text-[10px] font-bold font-semibold tracking-wider text-neutral-500 uppercase dark:text-neutral-400">
                                        Link Modul *
                                    </label>
                                    <input
                                        type="url"
                                        ...
                                        required
                                    />
                                    {prosesForm.errors.link_modul && (
                                        <p className="mt-1 font-sans text-[10px] text-rose-500">{prosesForm.errors.link_modul}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {prosesForm.data.status === 'Selesai' && (
                                <div>
                                    ...
                                    <input
                                        ...
                                        required
                                    />
                                    {prosesForm.errors.tanggal_realisasi && (
                                        <p className="mt-1 font-sans text-[10px] text-rose-500">{prosesForm.errors.tanggal_realisasi}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {prosesForm.data.status === 'Hold' && (
                                <div>
                                    ...
                                    <input
                                        ...
                                        required
                                    />
                                    {prosesForm.errors.tanggal_kebutuhan_baru && (
                                        <p className="mt-1 font-sans text-[10px] text-rose-500">{prosesForm.errors.tanggal_kebutuhan_baru}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {['Selesai', 'Hold', 'Batal'].includes(prosesForm.data.status) && (
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        required
                                    />
                                    {prosesForm.errors.reject_reason && (
                                        <p className="mt-1 font-sans text-[10px] text-rose-500">{prosesForm.errors.reject_reason}</p>
                                    )}
                                </div>
                            )}
                        </div>
                        ...
                        <DialogFooter className="pt-2">
                            <Button type="button" variant="outline" onClick={() => setProsesItem(null)}>
                                Batal
                            </Button>
            ...
            >
                <DialogContent className="max-w-sm">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Yakin ingin menghapus pengajuan <span className="font-bold text-neutral-900 dark:text-neutral-100">{deleteItem?.id}</span>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleDelete} className="bg-rose-600 text-white hover:bg-rose-700">
                            Hapus
                        </Button>
                    </DialogFooter>
                </DialogContent>
                ...
                <DialogContent className="max-h-[85vh] max-w-xl overflow-y-auto">
                    ...
                    {detailItem && (
                        <div className="mt-2 space-y-5 text-xs text-neutral-600 dark:text-neutral-400">
                            ...
                            <div className="grid grid-cols-2 gap-3.5 rounded-xl border border-neutral-100 bg-neutral-50 p-3.5 sm:grid-cols-3 dark:border-neutral-800/80 dark:bg-neutral-900/40">
                                <div>
                                    <p className="mb-0.5 font-sans text-[10px] font-bold tracking-wider text-neutral-400 uppercase">Tipe Pengajuan</p>
                                    <Badge
                                        variant="secondary"
                                        className="rounded-md bg-neutral-200/60 px-2 py-0.5 text-[10px] font-semibold text-neutral-800 dark:bg-neutral-800 dark:text-neutral-200"
                                    >
                                        {detailItem.type}
                                    </Badge>
                                </div>
                                <div>
                                    <p className="mb-0.5 font-sans text-[10px] font-bold tracking-wider text-neutral-400 uppercase">Status</p>
                                    <Badge
                                        className={`rounded-md border-0 px-2 py-0.5 text-[10px] font-semibold ${STATUS_COLORS[detailItem.status]}`}
                                    >
                                        {detailItem.status}
                                    </Badge>
                                </div>
                                <div>
                                    <p className="mb-0.5 font-sans text-[10px] font-bold tracking-wider text-neutral-400 uppercase">Pengaju</p>
                                    <p className="font-semibold text-neutral-900 dark:text-neutral-100">{detailItem.applicant}</p>
                                </div>
                                <div>
                                    <p className="mb-0.5 font-sans text-[10px] font-bold tracking-wider text-neutral-400 uppercase">
                                        Tanggal Pengajuan
                                    </p>
                                ...
                                <div>
                                    ...
                                    <p className="font-semibold text-neutral-900 dark:text-neutral-100">
                                        {detailItem.deadlineFormatted ?? detailItem.deadline}
                                    </p>
                                </div>
                            </div>
                            ...
                            {detailItem.type === 'Modul Baru' && (
                                <div className="space-y-3 rounded-xl border border-blue-100 bg-blue-50/10 p-3.5 dark:border-blue-900/40 dark:bg-blue-950/5">
                                    <div className="grid grid-cols-2 gap-3 text-xs">
                                        <div>
                                            <p className="mb-0.5 font-sans text-[10px] font-bold tracking-wider text-blue-500 uppercase dark:text-blue-400">
                                                Kategori / Jenis Pelatihan
                                            </p>
                                        ...
                                        <div>
                                            ...
                                            <p className="font-semibold text-neutral-800 dark:text-neutral-200">
                                                {detailItem.training_days ? `${detailItem.training_days} Hari` : '-'}
                                            </p>
                                        </div>
                                    </div>
                                    {detailItem.description && (
                                        <div className="border-t border-blue-100 pt-2 dark:border-blue-900/20">
                                            <p className="mb-1 font-sans text-[10px] font-bold tracking-wider text-blue-500 uppercase dark:text-blue-400">
                                                Deskripsi / Permintaan Khusus
                                            </p>
                                            <p className="leading-relaxed font-normal whitespace-pre-line text-neutral-700 dark:text-neutral-300">
                                                {detailItem.description}
                                            </p>
                                        </div>
                                    )}
                            ...
                            {detailItem.type === 'Revisi Modul' && (
                                <div className="space-y-3 rounded-xl border border-violet-100 bg-violet-50/10 p-3.5 dark:border-violet-900/40 dark:bg-violet-950/5">
                                    {detailItem.related_module_id && (
                                        <div>
                                            <p className="mb-1 font-sans text-[10px] font-bold tracking-wider text-violet-500 uppercase dark:text-violet-400">
                                                Modul Existing yang Direvisi
                                            </p>
                                            <div className="rounded-lg border border-neutral-200/50 bg-white p-2.5 dark:border-neutral-800/80 dark:bg-neutral-950">
                                                <span className="block font-mono text-[10px] font-bold text-neutral-500">
                                                    {detailItem.relatedModuleCode}
                                                </span>
                                                <span className="block text-xs font-semibold text-neutral-800 dark:text-neutral-200">
                                                    {detailItem.relatedModuleTitle}
                                                </span>
                                                <span className="mt-0.5 block font-sans text-[10px] text-neutral-400">
                                                    Versi Aktif: Rev. {detailItem.relatedModuleRevision}
                                                </span>
                                            </div>
                                        </div>
                                    )}
                                    {detailItem.revision_reason && (
                                        <div className="border-t border-violet-100 pt-2 dark:border-violet-900/20">
                                            <p className="mb-1 font-sans text-[10px] font-bold tracking-wider text-violet-500 uppercase dark:text-violet-400">
                                                Alasan Perubahan
                                            </p>
                                            <p className="leading-relaxed font-normal whitespace-pre-line text-neutral-700 dark:text-neutral-300">
                            ...
                            {detailItem.type === 'Kebutuhan Khusus' && (
                                <div className="space-y-4">
                                    <div className="space-y-3 rounded-xl border border-teal-100 bg-teal-50/10 p-3.5 dark:border-teal-900/40 dark:bg-teal-950/5">
                                        <div className="grid grid-cols-2 gap-3 text-xs">
                                            <div>
                                                <p className="mb-0.5 font-sans text-[10px] font-bold tracking-wider text-teal-500 uppercase dark:text-teal-400">
                                                    Jenis Kebutuhan
                                                </p>
                                                <p className="font-semibold text-neutral-800 dark:text-neutral-200">
                                                    {detailItem.jenis_kebutuhan ?? '-'}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="mb-0.5 font-sans text-[10px] font-bold tracking-wider text-teal-500 uppercase dark:text-teal-400">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-semibold text-neutral-800 dark:text-neutral-200">
                                                    {detailItem.pre_post_test ?? 'Tidak'}
                                                </p>
                                            </div>
                                        ...
                                        {detailItem.description && (
                                            <div className="border-t border-teal-100 pt-2 dark:border-teal-900/20">
                                                <p className="mb-1 font-sans text-[10px] font-bold tracking-wider text-teal-500 uppercase dark:text-teal-400">
                                    ...
                                    {(detailItem.link_modul ||
                                        ...
                                        ['Selesai', 'Batal', 'Hold'].includes(detailItem.status)) && (
                                        <div className="space-y-3 rounded-xl border border-purple-100 bg-purple-50/10 p-3.5 dark:border-purple-900/40 dark:bg-purple-950/5">
                                            <div className="mb-1 text-xs font-bold font-semibold tracking-wider text-purple-600 uppercase dark:text-purple-400">
                                                Penyelesaian Pengajuan (Admin/Staf PD)
                                            </div>

                                            <div className="grid grid-cols-2 gap-3 text-xs">
                                                {detailItem.tanggal_realisasi && (
                                                    <div>
                                                        <p className="mb-0.5 font-sans text-[10px] font-bold tracking-wider text-purple-500 uppercase dark:text-purple-400">
                                                            Tanggal Realisasi
                                                        </p>
                                                        <p className="font-semibold text-neutral-800 dark:text-neutral-200">
                                                            {detailItem.tanggal_realisasi_formatted}
                                                        </p>
                                                    </div>
                                                )}
                                                {detailItem.tanggal_kebutuhan_baru && (
                                                    <div>
                                                        <p className="mb-0.5 font-sans text-[10px] font-bold font-semibold tracking-wider text-purple-500 uppercase dark:text-purple-400">
                                                            Tanggal Kebutuhan Baru
                                                        </p>
                                                        <p className="font-semibold text-neutral-800 text-purple-600 dark:text-neutral-200 dark:text-purple-400">
                                                            {detailItem.tanggal_kebutuhan_baru_formatted}
                                                        </p>
                                                    </div>
                                                )}
                                                {detailItem.link_modul && (
                                                    <div className="col-span-2">
                                                        ...
                                                        <a
                                                            ...
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="mt-0.5 flex items-center gap-1 text-xs font-semibold text-blue-600 hover:underline dark:text-blue-400"
                                                        >
                                                            <ArrowUpRight className="size-3.5" />
                                                            Buka Dokumen Modul
                                                        </a>
                                            ...
                                            {detailItem.rejectReason &&
                                                detailItem.status !== 'Baru' &&
                                                detailItem.status !== 'Drafting' &&
                                                detailItem.status !== 'Menunggu Approval' && (
                                                    <div className="border-t border-purple-100 pt-2 dark:border-purple-900/20">
                                                        <p className="mb-1 font-sans text-[10px] font-bold tracking-wider text-purple-500 uppercase dark:text-purple-400">
                                                            Keterangan Proses / Alasan
                                                        </p>
                                                        <p className="leading-relaxed font-normal whitespace-pre-line text-neutral-700 dark:text-neutral-300">
                                                            {detailItem.rejectReason}
                                                        </p>
                            ...
                            {detailItem.fileName ? (
                                <div>
                                    <p className="mb-1.5 font-sans text-[10px] font-bold tracking-wider text-neutral-400 uppercase">Dokumen PDF</p>
                                ...
                                !detailItem.link_modul && (
                                    <div className="rounded-xl border border-dashed border-neutral-200 bg-neutral-50/30 p-3.5 text-center dark:border-neutral-800">
                                        <p className="font-sans text-xs text-neutral-400">Belum ada dokumen PDF yang diupload.</p>
                                    </div>
                                )
                            )}

                            {['Ditolak', 'Batal'].includes(detailItem.status) &&
                                detailItem.rejectReason &&
                                detailItem.type !== 'Kebutuhan Khusus' && (
                                    <div className="rounded-xl border border-rose-200 bg-rose-50/50 p-3.5 dark:border-rose-900/40 dark:bg-rose-950/20">
                                        <p className="mb-1 font-sans text-[10px] font-bold tracking-wider text-rose-700 uppercase dark:text-rose-400">
                                            {detailItem.status === 'Batal' ? 'Alasan Pembatalan' : 'Alasan Penolakan dari Manager'}
                                        </p>
                                        <p className="text-xs leading-relaxed font-medium whitespace-pre-line text-rose-600 dark:text-rose-300">
                                            {detailItem.rejectReason}
                                        </p>
                                    </div>
                                )}
